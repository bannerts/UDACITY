...
def localize(colors,measurements,motions,sensor_right,p_move):
    ...
    def movement(p_in, motion, p_move):
        n_rows = len(p_in)
        n_columns = len(p_in[0])
        p_row = []
    ...
    def normalize(p_in):
        ...
        for i in range(n_rows):
            for j in range(n_columns):
                p_sum += p_in[i][j]
        return [[p_in[i][j]/p_sum for j in range(n_columns)] for i in range(n_rows)]
    
    def measurement(colors, p_in, measurement, p_sensor):
        n_rows = len(p_in)
        n_columns = len(p_in[0])
        p_row = []
        for i in range(n_rows):
